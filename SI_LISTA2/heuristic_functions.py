...
def mobility_of_discs(game, player_type, distance_fn, *_):
    condition1, condition2, target_cell = (BLACK_BASE_CONDITION_1, BLACK_BASE_CONDITION_2, WHITE_TARGET_CELL) if player_type == WHITE_PLAYER_CELL else (WHITE_BASE_CONDITION_1, WHITE_BASE_CONDITION_2, BLACK_TARGET_CELL)
    # return sum(
    #     len(game.generate_all_possible_moves(x, y)) 
    #     for x in range(SIZE_OF_BOARD)
    #     for y in range (SIZE_OF_BOARD)
    #     if game.board[x][y] == player_type and not (condition1(x, y) or condition2(x, y))
    # )
    score = 0
    for x in range(SIZE_OF_BOARD):
        for y in range(SIZE_OF_BOARD):
            if game.board[x][y] == player_type:
                if not (condition1(x, y) or condition2(x, y)):
                    score += len(game.generate_all_possible_moves(x, y)) 
                    score -= distance_fn((x,y), target_cell)
                else:
                    score += 20
    return score
...
def follow_as_close_to_the_diagonal(game, player_type, distance_fn, start_cell, end_cell):
    score = 0
    x1, y1 = start_cell
    # score += 0 if game.is_disc_in_home_base(player_type, start_cell) else 150 - abs(x1 - y1) * 10
    x2, y2 = end_cell 
    score += 150 - (abs(x2 - y2) - abs(x1 - y1)) * 10
    score += distance_fn(start_cell, end_cell)

    # print(score)
    x = distance_of_target_base(game, player_type, distance_fn, start_cell, end_cell)
    # print(x)

    return score + x  # czy coś jeszcze????
...
def move_of_the_farthest_cell_as_far_as_possible(_, player_type, distance_fn, start_cell, end_cell):
    ...
    # return distance_fn(start_cell, target_cell) + distance_fn(start_cell, end_cell)
    return 2 * distance_fn(start_cell, target_cell) - distance_fn(end_cell, target_cell)
